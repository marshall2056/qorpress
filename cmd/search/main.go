...
import (
	...
	srx "github.com/qorpress/qorpress/core/search"
...
var (
	vendor = flag.String("vendor", "manticore", "Specify the vendor you want to test ("+srx.VENDOR_ELASTIC+"/"+srx.VENDOR_BLEVE+"/"+srx.VENDOR_MANTICORE+")")
	...
	url          = flag.String("url", "", "ElasticSearch URL (i.e. 192.168.1.26:9200) - not relvant for Bleve")
	KVStore      = flag.String("store", bbadger.Name, "KV Store for Bleve. Avialble stores: "+goleveldb.Name+", "+boltdb.Name+", "+bbadger.Name)
)
...
func main() {
	...
	switch *action {
	case index:
		if *vendor == srx.VENDOR_ELASTIC && *url == "" {
...
func indexDocument(engine srx.SearchEngine) (timeTook int64) {
	data, err := utils.ReadFile(*pathToDoc)
	utils.ErrorCheck(err)
	fmt.Printf("Indexing document: %s\n", *pathToDoc)
	doc := srx.Document{utils.FixIdSyntax(*pathToDoc), data, ""}
	timeTook, err = engine.Index(&doc)
	utils.ErrorCheck(err)
	return timeTook
}

func indexDocuments(engine srx.SearchEngine) (int64, int) {
	// prepare documents for batch index
	files, err := ioutil.ReadDir(*pathToFolder)
	utils.ErrorCheck(err)
	var docCount int = 0
	var documents = make([]*srx.Document, len(files))
	for _, file := range files {
		...
		utils.ErrorCheck(err)
		documents[docCount] = &srx.Document{utils.FixIdSyntax(path), data, ""}
		docCount++
	}
	// index in batch
...
func getSearchEngine() srx.SearchEngine {
	engine, err := srx.GetSearchEngine(url, vendor, *KVStore)
	utils.ErrorCheck(err)
	return engine
}
